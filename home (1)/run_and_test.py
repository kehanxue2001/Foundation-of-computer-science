# Written by Eric Martin and modified by Han-Hsin Lin for COMP9021
...
class NamedTemporaryFile:
    ...
    """
    def __init__(self, mode='wb', dir=None, suffix="", delete=True):
        self._mode = mode
        self._delete = delete
        self._suffix = suffix
        if dir is None:
    ...
    def __enter__(self):
        ...
        file_name = os.path.join(self._dir, os.urandom(24).hex()+self._suffix)
...
def run_and_test(magic_arguments, outputs_inputs):
    ...
    if magic_arguments.startswith('-c'):
        ...
        if match:
            ...
            p = execute(('python', '-B', '-c', statements), options)
        ...
        with NamedTemporaryFile('w', dir='.') as temp_file:
            ...
            p = execute(('python', '-B', temp_file.name), options)
    if not p:
        return
